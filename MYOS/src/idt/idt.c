...
void isr80h_register_command(int command_id, ISR80H_COMMAND command)
{
    ...
    {
        panic("Command is out of bound\n");
    }

    if(isr80h_commands[command_id])
    {
        panic("Attempting to override an existing command\n");
    }
...
void* isr80h_handle_command(int command,struct interrupt_frame* frame)
{
    ...
    ISR80H_COMMAND commnd_func = isr80h_commands[command];
    if(!commnd_func)
    {
        return 0;
    }

    result = commnd_func(frame);
    return result;
}
